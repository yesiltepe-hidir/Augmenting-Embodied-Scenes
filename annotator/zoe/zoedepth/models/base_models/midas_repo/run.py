...
import glob
import torch
import utils.processor_utils as processor_utils
import cv2
import argparse
import time
...
def run(input_path, output_path, model_path, model_type="dpt_beit_large_512", optimize=False, side=False, height=None,
    ...
    if input_path is not None:
        ...
        for index, image_name in enumerate(image_names):
            ...
            # input
            original_image_rgb = processor_utils.read_image(image_name)  # in [0, 1]
            ...
            if output_path is not None:
                ...
                if not side:
                    processor_utils.write_depth(filename, prediction, grayscale, bits=2)
                else:
                    original_image_bgr = np.flip(original_image_rgb, 2)
                    content = create_side_by_side(original_image_bgr*255, prediction, grayscale)
                    cv2.imwrite(filename + ".png", content)
                processor_utils.write_pfm(filename + ".pfm", prediction.astype(np.float32))
